...
const PopUp = () => {
  ...
  const deleteDataById = async (id: number) => {
    try {
      const response = await axios.delete(`http://localhost:8000/api/complaints/${id}`);
  ...
  return (
    <div>
      <div>
        <div
          className={`bg-white w-[363px] h-[312px] rounded-md border shadow-2xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 animate__animated animate__fadeIn ${state.actionHapusPengaduan ? "" : "hidden"
            }`}
        >
          ...
          <div className="w-full flex justify-center mt-[20px]">
            <button
              onClick={() =>
                state.getId !== undefined &&
                deleteDataById(state.getId)
              }
