...
const BalasPengaduan = () => {
    ...
    try {
      ...
      if (data) {
        ...
        data.forEach((respon) => {
          ...
          setResponEdit(respon.respon);
          setIdRespon(respon.id);
        });
      }
    } catch (err) {
  ...
  const fetchDataById = async (id: number) => {
    try {
      ...
      data.forEach((item) => {
        ...
        console.log("waktu sekarang : ", item.created_at);
      });
    } catch (err) {
      console.error("Error fetching data:", err);
      return null;
  ...
  const addRespon = async (e: any) => {
    e.preventDefault(); // Memindahkan pencegahan default event ke awal

    // Cek jika ada data respon yang sudah ada
    const hasExistingRespon = dataRespon.some(
      (respon) => respon.respon !== "" && respon.respon !== null
    );

    if (hasExistingRespon) {
      alert("Sudah ada respon");
    ...
    try {
      ...
      const { error } = await supabase.from("respon_superadmin").insert([
        {
          respon: valueRespon,
  ...
  const editRespon = async (e: any) => {
    ...
    try {
      const { data, error } = await supabase
        .from("respon_superadmin")
        .update({ respon: responEdit, created_at: formattedTime })
        .eq("id", idRespon);
      ...
      <div className="scrollbar-hide w-full overflow-auto ">
        <div className=" mt-10 ">
          ...
          <div className="w-[1230px]  border-[#f0f0f0] rounded-xl mt-1 p-5 border-2 ">
            ...
            <div
              className={`mt-7 ${editToggle ? "hidden" : ""} ${
                responAdmin !== "" ? "hidden" : ""
              }`}
            >
